...
def get_data_raw(dataloader, discretizer, normalizer):
    data_raw = {}
    ...
    data_raw['data'] = [[Xs, masks], ys]
    data_raw['names'] = names
    data_raw['ts'] = ts

    return data_raw
class BatchGenDeepSupervision(object):

    def __init__(self, data_raw, batch_size, shuffle, return_names=False):
        # print('BatchGenDeepSupervision __init__')
        self.data = data_raw['data']
        self.names = data_raw['names']
        self.ts = data_raw['ts']
        self.batch_size = batch_size
        self.shuffle = shuffle
        self.return_names = return_names
    ...
    def _generator(self):
        # print('BatchGenDeepSupervision _generator')
        B = self.batch_size
        while True:
            if self.shuffle:
                N = len(self.data[1])
    ...
    def __iter__(self):
        # print('BatchGenDeepSupervision __iter__')
        return self.generator

    def next(self):        
        # print('BatchGenDeepSupervision next')
        with self.lock:
            return next(self.generator)
...
def save_results(names, ts, pred, y_true, path):
    # print('BatchGenDeepSupervision save_results')
